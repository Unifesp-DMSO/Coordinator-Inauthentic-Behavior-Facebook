...
def replace_expressions(tweet, language):
	...
	with open('./SNAPKG/Common/data/PreProcessor/Universal.csv', newline='') as csvfile:
		spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
		for row in spamreader:
			tweet = re.sub(	row[0]	,	row[1]	,	tweet	)


	with open('./SNAPKG/Common/data/PreProcessor/Names.csv', newline='') as csvfile:
		spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
		for row in spamreader:
			tweet = re.sub(	row[0]	,	row[1]	,	tweet	)
	...
	if language=="pt":
		...
		with open('./SNAPKG/Common/data/PreProcessor/PT.csv', newline='') as csvfile:
			spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
			for row in spamreader:
				tweet = re.sub(	row[0]	,	row[1]	,	tweet	)	
	...
	if language=="es":
		...
		with open('./SNAPKG/Common/data/PreProcessor/ES.csv', newline='') as csvfile:
			spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
			for row in spamreader:
				tweet = re.sub(	row[0]	,	row[1]	,	tweet	)	
	...
	if language=="en":
		...
		with open('./SNAPKG/Common/data/PreProcessor/EN.csv', newline='') as csvfile:
			spamreader = csv.reader(csvfile, delimiter=',', quotechar='"')
			for row in spamreader:
				tweet = re.sub(	row[0]	,	row[1]	,	tweet	)	
...
def remove_stopWords(tweet,language):
	...
	if t:
		if language=="pt":
			...
			f				=	open("./SNAPKG/Common/data/PreProcessor/custom_stopwords_PT.txt")

		if language=="es":

			stop			=	stopwords.words('spanish')
			stop			=	[ x.upper() for x in stop]
			f				=	open("./SNAPKG/Common/data/PreProcessor/custom_stopwords_ES.txt")
			
		if language=="en":

			stop			=	stopwords.words('english')
			stop			=	[ x.upper() for x in stop]
			f				=	open("./SNAPKG/Common/data/PreProcessor/custom_stopwords_EN.txt")
			

		languageStopWords	=	f.read().splitlines()
		f.close()

		f					=	open("./SNAPKG/Common/data/PreProcessor/universal_stopwords.txt")
